...
def train_and_evaluate_linear_regression(dataframe, test_size=0, random_state=0):
    """
...
# Check the model Y = motor_updrs và 16 independent variables
...
# 
# Check the model Y = total_updrs và 18 independent variables
test_sizes = [0.5, 0.4, 0.3, 0.2]
...
# Define a function for evaluating a model with k-fold cross-validation
def train_linear_regression(dataframe):
    x = dataframe.iloc[:, 1::]
    y = dataframe.iloc[:, 0]
...
# Define a function for evaluating a model with k-fold cross-validation
def evaluate_with_cross_validation(model, x, y, k=5):
...
motor_model = train_linear_regression(motor_up_df)
motor_cross_val_results = evaluate_with_cross_validation(motor_model, motor_up_df.iloc[:, 1::], motor_up_df.iloc[:, 0], k)

total_model = train_linear_regression(total_up_df)
total_cross_val_results = evaluate_with_cross_validation(total_model, total_up_df.iloc[:, 1::], total_up_df.iloc[:, 0], k)

print(f"Performance Metrics (Motor) - {k}-fold Cross-Validation:")
print(motor_cross_val_results)

print(f"Performance Metrics (Total) - {k}-fold Cross-Validation:")
print(total_cross_val_results)